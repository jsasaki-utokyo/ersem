...
ホストモデル（GOTM）の設定変更も推奨：

| ファイル | 変更内容 |
|---------|---------|
| `gotm.yaml` | `ode_method: fe`（前進オイラー法）を `ode_method: emp2`（拡張修正Patankar 2次精度）に変更 |
...
### 修正3: ODEソルバーを前進オイラー法からEMP2に変更（`gotm.yaml`）

```yaml
# 修正後
ode_method: emp2  # 拡張修正Patankar法、2次精度
```

EMP2ソルバーの特長：
- 構造的に全状態変数の非負性を保証
- 2次精度（前進オイラー法の1次精度に対して）
- 増幅カスケードを引き起こす数値的なアンダーシュート/オーバーシュートを防止
